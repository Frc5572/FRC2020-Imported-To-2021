...
void Robot::AutonomousPeriodic() { 
    ...
    while(m_timer.Get() > 14 && m_timer.Get() < 15)
    {
    ...
    while(m_timer.Get() > 15 && m_timer.Get() < 16.5)
    {
        m_rightBottomMotor.Set(-.3);
        m_rightMiddleMotor.Set(-.3);
    ...
    while(m_timer.Get() > 16.5 && m_timer.Get() < 17.5)
    {
        m_rightBottomMotor.Set(0);
        m_rightMiddleMotor.Set(0);
    ...
    while(m_timer.Get() > 17.5 && m_timer.Get() < 20.5 ){
        //shooter.AutoPID();
        m_leftShooter.Set(.6); 
        m_rightShooter.Set(.6); 
        shooterHood.Set(frc::DoubleSolenoid::Value::kForward);
    ...
    while(m_timer.Get() > 20.5 && m_timer.Get() < 27.5 ){
        m_hopper.Set(.3);
        m_leftShooter.Set(.6); 
        m_rightShooter.Set(.6); 
        shooterHood.Set(frc::DoubleSolenoid::Value::kForward);
    }
    while(m_timer.Get() > 27.5 && m_timer.Get() < 28.5){ 
        shooterHood.Set(frc::DoubleSolenoid::Value::kReverse);
        m_leftShooter.Set(0);
        m_rightShooter.Set(0);
        m_hopper.Set(0.0);
